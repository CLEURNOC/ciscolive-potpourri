...
def main():
    ...
    parser.add_argument(
        ...
        help="Whether or not to have OSPF use passive interface (default: SVI will be a passive interface)",
        action="store_true",
    )
    parser.add_argument(
    ...
    command = [
        ...
        "-k",
        "-e",
        "vlan_name={}".format(args.vlan_name),
        "-e",
        "vlan_id={}".format(args.vlan_id),
        "-e",
        "ansible_python_interpreter={}".format(sys.executable),
        "-e",
        "@{}".format(cred_file.name),
        "-e",
        "build_v4={}".format(build_v4),
        "-e",
        "build_v6={}".format(build_v6),
        "-e",
        "is_stretched={}".format(is_stretched),
        "-e",
        "generate_iflist={}".format(generate_iflist),
        "-e",
        "ospf_type={}".format(ospf_type),
        "-e",
        "add_acl={}".format(add_acl),
    ...
    if args.svi_v4_network:
        command += [
            "-e",
            "svi_v4_prefix={}".format(svi_prefix),
            "-e",
            "svi_subnet_len={}".format(args.svi_subnet_len),
            "-e",
            "svi_v4_network={}".format(args.svi_v4_network),
        ]
    if args.svi_v6_network:
